...
if strcmpi(out_format,"Matlab")
    save(fullfile(CyGoutpath, [project_name '_' daterangechar '.mat']), ...
        ...
        'incidenceAngleDeg', 'rxAntennaGain', 'EIRP_L1', 'SNR_L1_L', ...
        'reflectivityLinear_L1_L', 'KURTOSIS', ...
        'KURTOSIS_DOPP_0', 'TE_WIDTH', 'DDM_NBRCS','powerAnalogW', ...
        'qualityControlFlags', 'noiseFloorCounts','REFLECTIVITY_PEAK_L1_L', ...
        'receivingAntenna', 'qualityControlFlags_2', 'spAzimuthAngleDegNorth', 'coherencyRatio', ...
...
elseif strcmpi(out_format,"netcdf")
    % Create NetCDF file
        ...
        netcdf.putAtt(netcdf_cyg, var_rxAntennaGain, 'rxAntennaGain', 'Receiver antenna gain toward specular point [dB]');

        var_rxAntennaGain = netcdf.defVar(netcdf_cyg,'powerAnalogW','NC_DOUBLE',dimid);
        netcdf.putAtt(netcdf_cyg, var_rxAntennaGain, 'powerAnalogW', 'Peak power of the DDM. It the true power that would have been measured by an ideal (analog) power sensor and corrected for quantization effects [Watt]');
        ...
        netcdf.putAtt(netcdf_cyg, var_qualityControlFlags, 'qualityControlFlags', 'Quality check flags of the observation [int32]');
        
        var_noiseFloorCounts = netcdf.defVar(netcdf_cyg,'noiseFloorCounts','NC_DOUBLE',dimid);
        netcdf.putAtt(netcdf_cyg, var_noiseFloorCounts, 'noiseFloorCounts', 'Noise floor of the DDM [Watt]');
