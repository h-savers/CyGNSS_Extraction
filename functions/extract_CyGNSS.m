...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [mission,L1b_product,L1b_product_version,UTC_Time,DoY,SoD,receivingSpacecraft,transmittingSpacecraft,pseudoRandomNoise, ...
        spAzimuthAngleDegOrbit,specularPointLat,specularPointLon,incidenceAngleDeg,rxAntennaGain, EIRP_L1,SNR_L1_L, ...
    ...
    EIRP_L1=[];                                % EIRP_L1 [W]
    SNR_L1_L=[];                                 % SNR of reflected signal - NOTE: calculated from the uncalibrated DDM in counts [dB]
    powerAnalogW=[];                                  % peak power
    noiseFloorCounts=[];                                  % noise floor
    RXRANGE=[];                             % Rx range [m]
    TXRANGE=[];                             % Tx range [m]
    NST=[];                                 % overall quality
...
%%%%%%%%%%%%%%%%%%%%% added by Mauro
    REFLECTIVITY_PEAK_L1_L=[] ;                  % Reflectivity in lienar units from L1b producxt  
    ...
    spAzimuthAngleDegNorth=[] ;
    coherencyRatio=[] ;                    % Coherency ration from L1b produt
    DDM_LES=[]  ;                           % DDM_LES from L1b product
    powerRatio=[] ;                                 % Power ration from Mohammad M. Al-Khaldi et al., 2021
    ...
    for jj=1:nsat     % loop on  the 8 satellites   
        ...
        if ~isempty(chkfile)    
            ...
            disp(['% reading satellite ' num2str(jj) ' - file ' infile ])
             [mission, L1b_product, L1b_product_version, sp_lat,sp_lon,theta,scid,strans,ts,nst_full,prn,azimuth_angle,sp_rx_gain, ...
             eirp,snr,nf,rxrange,txrange,ddm_nbrcs,qc,pa,Reflectivity_linear,Kurtosis,...
             Kurtosis_dopp0, brcs, reflectivity_peak, receivingantenna, qc_2, sp_azimuth_angle_deg_north, coherencyratio, ddm_les]=readnc_CyGNSS_v2(inpath,infile,lambda,Doppler_bins,savespace); 
        ...
        % cat variables
            disp('% cat variables ')
            receivingSpacecraft=cat(1,receivingSpacecraft,scid(:));
            ...
            EIRP_L1=cat(1,EIRP_L1, eirp(:));
            SNR_L1_L=cat(1,SNR_L1_L, snr(:));
            qualityControlFlags=cat(1,qualityControlFlags, qc(:)); 
            powerAnalogW=cat(1,powerAnalogW, pa(:));
            noiseFloorCounts=cat(1,noiseFloorCounts, nf(:));
            ...
            REFLECTIVITY_PEAK_L1_L=cat(1,REFLECTIVITY_PEAK_L1_L,reflectivity_peak);   
            receivingAntenna=cat(1,receivingAntenna,receivingantenna);
            spAzimuthAngleDegNorth=cat(1,spAzimuthAngleDegNorth);
            qualityControlFlags_2=cat(1,qualityControlFlags_2, qc_2);
            spAzimuthAngleDegNorth=cat(1,spAzimuthAngleDegNorth,sp_azimuth_angle_deg_north);
            coherencyRatio=cat(1,coherencyRatio,coherencyratio);
            DDM_LES=cat(1,DDM_LES,ddm_les);    
            powerRatio=cat(1,powerRatio, pr) ; 
        else
